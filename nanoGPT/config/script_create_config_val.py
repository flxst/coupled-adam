...
def get_config_paths(exp: str, filter: str, platform: str) -> Tuple[Dict[str, str], str]:
    ...
    list_of_files = [elem for elem in os.listdir(config_directory) if elem.startswith(f'exp{exp}-{platform}')]
    if filter != '':
        list_of_files = [elem for elem in list_of_files if filter in elem]
    assert len(list_of_files) == 1, f"ERROR! could not find a single config file exp{exp}-{platform}* in {config_directory}"
    train_config_name = list_of_files[0] 
    val_config_name = list_of_files[0].replace('exp', 'val')
    config_paths = {
...
def get_last_checkpoint(directory: str, train_config_name: str) -> str:
    ...
    list_of_files = [elem for elem in os.listdir(checkpoint_directory) if elem.startswith(train_config_name) and not 'ckpt_step_' in elem]
    ckpt_steps = [int(elem.split("ckpt_step=")[-1].split("_valloss")[0]) for elem in list_of_files]
...
def main(_args):
    ...
    config_paths, train_config_name = get_config_paths(_args.exp, _args.filter, _args.platform)
    ...
    lines = [
        "# --- val ---\n",
        "init_from = 'resume'\n",
        f"init_directory = '{_args.directory}/{last_checkpoint.split('---')[0]}'\n",
