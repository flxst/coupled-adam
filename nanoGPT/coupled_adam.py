...
__all__ = ["CoupledAdam", "coupledadam"]


class CoupledAdam(Optimizer):
    def __init__(
        self,
        params: ParamsT,
...
CoupledAdam.__doc__ = r"""Implements AdamW algorithm.
...
def _single_tensor_adamw(
    ...
    *,
    ...
    if True:
        raise RuntimeError("FUNC = single")
    ...
    for ((
        ...
        if capturable:
            ...
            if amsgrad:
                ...
                ### COUPLED ADAM START
                for i in range(len(device_exp_avg_sqs)):
                    tensor = device_exp_avg_sqs[i]  # shape = [V, H]
                    mean_tensor = avg_scale * torch.mean(tensor, dim=0)  # shape = [H]
                    mean_tensor_broadcasted = torch.broadcast_to(mean_tensor, tensor.shape)  # shape = [V, H]
                    device_exp_avg_sqs[i] = mean_tensor_broadcasted
                ### COUPLED ADAM END
                exp_avg_sq_sqrt = torch._foreach_sqrt(device_exp_avg_sqs)
